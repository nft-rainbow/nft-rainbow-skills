...
2. **KYC 前置：** 未完成 KYC 时，`POST /v1/login` 等可能返回 `KYC required` / `40105`；先按 `kyc.md` 完成审核，再继续鉴权与集成。
3. **JWT：** KYC 通过后创建 App 并拿到 `app_id`/`app_secret`，再 login 换 token；过期用 refresh；勿把 secret 写入仓库（见 `auth.md`）。

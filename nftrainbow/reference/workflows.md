...
开始前先完成 KYC（见 [kyc.md](kyc.md)），再完成鉴权前置（创建 App → `app_id`/`app_secret` → login → Bearer，见 [auth.md](auth.md)）。
...
1. 先完成 KYC（见 [kyc.md](kyc.md)），再完成鉴权前置（见 [auth.md](auth.md)；含 login 与 Bearer）。
2. 任选：

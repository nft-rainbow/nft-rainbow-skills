...
0. 完成鉴权前置（见 [auth.md](auth.md)；含 login 与 Bearer）。
1. 任选：
...
2. `GET /v1/mints/{id}` 轮询至 success/failed。
3. 失败可评估 `POST /v1/mints/{id}/reMint`（见 swagger 说明）。
...
0. 完成鉴权前置（见 [auth.md](auth.md)；含 login 与 Bearer）。
1. `POST /v1/contracts` 部署 ERC721/ERC1155（`chain`/`name`/`symbol`/`type`）。
2. 等待合约 `status=1`；记录 `address`。
3. **代付：** `POST /v1/contracts/{address}/sponsor` 和/或自动代付配置 `.../config/auto-sponsor`。控制台说明：https://docs.nftrainbow.xyz/tutorials/guides/kong-zhi-tai-he-yue-dai-fu-she-zhi
4. 可选：`POST /v1/files/oss` → `POST /v1/metadata` 得到 metadata URI。
5. `POST /v1/mints`（CustomMint）或 batch；轮询任务状态。
